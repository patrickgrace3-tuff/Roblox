...
- A third **dungeon** zone (further along X, its own `DungeonSpawn`) is a torch-lit stone corridor: grab the **sword and shield** from the weapon rack near the entrance, then use number-key abilities — **1** toggles auto-swing (your sword keeps swinging at nearby/facing enemies every 0.6s until you press 1 again), **2** activates a 3-second, 100% damage-immunity shield. A health bar and an ability status line show bottom-left/bottom-right. Every mob has its own floating health bar above its head (with a name label for the boss) that shrinks as you damage it. Chasing mobs are scattered down the corridor, and it ends in a boss room guarded by **Nonna Fury**, an oversized angry chef wielding a giant spoon — defeating her spawns a golden victory pedestal.
